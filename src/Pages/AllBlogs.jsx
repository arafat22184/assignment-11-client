...
//   return (
//     <section className="bg-slate-950 min-h-screen py-20 px-4 lg:px-6">
...
//           {/* Category Dropdown */}
//           <select
//             className="w-full lg:w-60 px-4 py-2 bg-slate-800 text-white border border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer"
...
//             <button
//               type="submit"
//               className="bg-blue-500 hover:bg-blue-600 text-white px-5 py-3 rounded-r-full cursor-pointer"
//             >
...
//           <p className="text-center text-slate-400 text-6xl mt-12">
//             No blogs found.
//           </p>
//         ) : (
//           <motion.div
...
const AllBlogs = () => {
  ...
  useEffect(() => {
    ...
    debounceTimer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_LINK}/blogs?search=${searchText}`
        );
        const data = await res.json();
        setBlogs(data);
      } catch (error) {
  ...
  return (
    <section className="bg-slate-950 min-h-screen py-20 px-4 lg:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Blog Grid */}
        {filteredBlogs.length === 0 ? (
          <p className="text-center text-slate-400">No blogs found.</p>
        ) : (
          <motion.div
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 min-h-[200px]"
            initial="hidden"
